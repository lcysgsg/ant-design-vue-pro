<template>
  <a-popover v-model="active" trigger="click" placement="bottomLeft" arrowPointAtCenter>
    <AIcon type="setting" style="cursor: pointer" />
    <div class="d2p-drawer-wrapper" slot="content">
      <div class="component--list">
        ...
        <a-divider style="margin: 8px 0 12px"></a-divider>

        <draggable ghost-class="ghost" handle=".handle" v-model="currentValue" @change="onDraggableChange">
          <div
          ...
          >
            <a-icon
              type="swap"
              style="cursor: all-scroll; margin-right: 5px; transform: rotate(90deg)"
              class="component--list-item-handle handle"
            />
            <a-checkbox style="flex: 1" :checked="currentValue[index].show" @change="onCheckChange($event, option)">
              {{ option.title || option.dataIndex || _text.unnamed }}
            </a-checkbox>
...
import D2TableColumnsFixedController from './ColumnsFixedController'
  ...
  methods: {
    ...
    onCheckAllChange () {
      ...
      this.currentValue = this.currentValue.map((e) => {
        e.show = this.checkAll
        return e
      })
      this.submit()
    },
    onCheckChange (e, option) {
      option.show = e.target.checked
      this.checkAll = this.currentValue.filter((e) => e.show).length === this.currentValue.length
      this.submit()
    },
    ...
    refresh () {
      ...
      compareSource.forEach((option) => {
        ...
        if (this.storedOptions != null) {
          storeItem = option
          option = options.find((column) => column.dataIndex === option.dataIndex)
        }
        let item = value.find((column) => column.dataIndex === option.dataIndex)
    ...
    onDraggableChange (added, removed, moved) {
      this.submit()
    }
  }
}
